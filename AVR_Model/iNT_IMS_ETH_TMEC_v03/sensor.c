...
float iSmoothlyADC(int  viADCCh_arg){
    float viDataBuffer = 0.0;
    int viLoopLimit = 20; 
    int i=0;
    
    for(i=0;i<viLoopLimit;i++){
        viDataBuffer += read_adc(viADCCh_arg);
    }
      
    return  (float)(viDataBuffer/((float)viLoopLimit));
}
/*============================================================*/
...
//                                        sensor->value = (epPT100TempSlope * adcData[ch]) + epFactor_C[ch];   
                                    ...
                                    break;      
        case CAL_EXTLINEAR      :   sensor->value   =   (epFactor_B[ch]   * adcData[ch]) + epFactor_C[ch];  
                                    break;
        case CAL_EXTPOLYNOMIAL  :   sensor->value   =   (epFactor_A[ch]   * adcData[ch] * adcData[ch]) + 
