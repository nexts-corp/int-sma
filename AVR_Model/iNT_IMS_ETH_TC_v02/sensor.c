...
float iReadSmoothlyADC(int ch){
     ...
     return (float)(viADCbuffer/viSmoothlyValue);
}
...
//             channelConnect  = SENS_NORMAL;
//         }  
//         viRiaseUpVoltValue = (float)((iUpdateSmoothlyADC(ch)+0.213)-viZeroSet);
...
//         iTCVoltData[ch] = viRiaseUpVoltValue; 
         ...
         if(viVoltUseCaluate < TMCP_SHORT_LIMIT){
             //channelConnect  = SENS_ERR_SHORT;  
             channelConnect  = SENS_ERR_DISCONNECT;
         }else if(viVoltUseCaluate > TMCP_OPEN_LIMIT){
             channelConnect  = SENS_ERR_DISCONNECT;
         }else{
             channelConnect  = SENS_NORMAL;
         }
...
//                                         sensor->value = iAD595VoltToTemp((float)((viRawVoltValue+0.213)-viZeroSet));        //0.213 *2 = 0.426       
                                         ...
                                         viRiaseUpVoltValue = (float)((iUpdateSmoothlyADC(ch))+0.213);
                                         viVoltUseCaluate = (viRiaseUpVoltValue-viZeroSet);
                                         sensor->value = iAD595VoltToTemp(viVoltUseCaluate);        //0.213 *2 = 0.426
                                    }                
                                    else if(TEMP_SEL == TEMP_TMEC){                 
...
//             epADCRef1[ch] = iAD595ADCToTemp(viADCbuffer,30);       
             ...
             viRiaseUpVoltValue = (float)((iUpdateSmoothlyADC(ch))+0.213);
             viVoltUseCaluate = (viRiaseUpVoltValue-viZeroSet);
             epADCRef1[ch] = iAD595VoltToTemp(viVoltUseCaluate);        //0.213 *2 = 0.426
        }else{
