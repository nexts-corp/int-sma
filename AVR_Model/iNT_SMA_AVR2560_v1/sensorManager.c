...
iChar_t iSensorRead(iDataMini_t *pviOutData_arg){
    ...
    for(i=0;i<5;i++){
       ...
       if(viDVConfigUpdate.viOperate.viSensorEnable[i]==1){
           viDataReportBuff.viChannelID = viDVConfigUpdate.viSensorConfig[i].viChannelID;
           viDataReportBuff.viSensorID = viDVConfigUpdate.viSensorConfig[i].viSensorID;
           viDataReportBuff.viUnitID =   viDVConfigUpdate.viSensorConfig[i].viUnitID; 
           iSensorReadByType(&viDataReportBuff);
           memcpy(&pviOutData_arg->value[viIndexWrite],viCMD,2);
           viIndexWrite += 2;
           memcpy(&pviOutData_arg->value[viIndexWrite],&viDataReportBuff,sizeof(viDataReportBuff));
           viIndexWrite += sizeof(viDataReportBuff);
