    ...
    if(iFSize((char const * const)viDirDataPath,(char const * const)viDataFName,&viFileSize)==1){ 
       if(viFileSize == 0){
          ...
          if(viLimitRecord_arg<viLineID){
              for(i=0,j=0;j<viLimitRecord_arg;i++){ 
                  #asm("wdr")
                  ...
                  if(viDataBuff[2] == 'S'){
                     //continue;
                  }else if((viDataBuff[2] == 'N') || (viDataBuff[2] == 'Y') || (viDataBuff[2] == 'L')){
                     //printDebug("[iDataSelectAll]Record id(%ld),status send(%c).\r\n",viRecordIDBuff,viDataBuff[2]);
//                     pviOutData_arg[i] = viRecordIDBuff;
//                     *pviOutLength_arg += 1;
                       viRIDBuff[j] = viRecordIDBuff;
                       viRIDLength += 1; 
                       j++;
                  }
                  indexReadFile = (DB_BLOCK_BUFFER_def*(i+1));
              }
//              printDebug("[iDataSelectAll]RID count(%ld).\r\n",*pviOutLength_arg);  
//              printDebug("[iDataSelectAll]Data:[\r\n");
              printDebug("[iDataSelectToSettlement]RID count(%ld).\r\n",viRIDLength);  
              printDebug("[iDataSelectToSettlement]Data:[\r\n");
              viIndexWrite = 0; 
              for(i=0;i<viRIDLength;i++){
//                 memcpy(&pviOutData_arg[viIndexWrite],&viRIDBuff[viIndexWrite],2);
//                 viIndexWrite += 2;  
