...
void iTerAckByTagB(iChar_t viTagName, iChar_t viPropNumber){              //host requst = A , terminal ack = B
     ...
     iChar_t viTID[8] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
     ...
     if(pviDataTX!=NULL && pviDataRX!=NULL){
         ...
         iGetTID_ByteFormat(viTID);
         print_payload(viTID,sizeof(viTID));
         if(viTagName==TAG_4A){ 
...
void iTagParser(iData_t *pviData_arg){
    ...
    if(pviData_arg->value[0] == 0x00){
        ...
        switch(pviData_arg->value[2]){
             case CONF_NETWORK:{      //config properties
                printDebug("[iTagParser]Host need to config(Network)\r\n");
                break;
             } 
             case CONF_PROPERTIES:{      //config properties
                printDebug("[iTagParser]Host need to config(Properties),len(%d)\r\n",viDataLen);
...
void iGetTID_ByteFormat(iChar_t *pviTID_arg){
    char tid[8] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}; 
