...
void vGSM_PowerOff( void ) { 
	...
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 50 ) == GSM_ENOERR ) { /* Power off GSM module */ 
		...
		delay_ms( 1000 ); GPIO_ResetBits( GSM_SWITCH_PORT, GSM_SWITCH_PIN ); 
	} 
}
...
//  	uint8_t ESC[2] = {0x33,0x00};
	...
	vGSM_SendATcommand( ( uint8_t* )"AT+IPR=115200\r\n", 15 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 5 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+IPR error\r\n" );
	vGSM_SendATcommand( ( uint8_t* )"AT&W\r\n", 6 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 5 ) != GSM_ENOERR )
		printDebug_Semphr( DETAIL,"[GSM_TASK]: AT&W error\r\n" );
	
	vGSM_SendATcommand( ( uint8_t* )"AT+QICSGP=1,\"PWA.SCADA\"\r\n", 25 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 5 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+QICSGP=1 error\r\n" );
	vGSM_SendATcommand( ( uint8_t* )"AT+QISCON\r\n", 11 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 5 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+QISCON error\r\n" );
