...
/**
  ...
  * @retval Status
	...
	*					 - 0x02 Malloc failed
  */
uint8_t ucPKTQUEUE_Add(PACKET_QUEUE *packetQueue, uint8_t *queueCounter, uint16_t id, uint8_t *packet, uint8_t length, uint16_t timeout) {
...
//	printDebug_Semphr(INFO, "ok2 ");
	...
	*queueCounter += 1;                              
	printDebug_Semphr(INFO, "%80s<PKTQUEUE_Add> FID[%04X] (num:%d)\r\n", "", id, *queueCounter);         
	return 0x00; 
}
/*==========================================================================================*/
/**
  ...
  */
uint8_t ucPKTQUEUE_Remove(PACKET_QUEUE *packetQueue, uint8_t *queueCounter, uint16_t id) {
...
//			packetQueue[i].packetData  = NULL;
			...
			packetQueue[i].retryTime   = 0;    
			*queueCounter -= 1; 																																 
			printDebug_Semphr(INFO, "%80s<PKTQUEUE_Remove> FID[%04X] \r\n", "", id); 
			return 0;         
		}
	}
	printDebug_Semphr(MINOR_ERR, "%80s<PKTQUEUE_Remove> FID[%04X] not found!!\r\n", "", id);
	/* Queue not found */
	return 0x01;
}
/*==========================================================================================*/
	...
	for(i = 0; i < MAX_PKT_QUEUE; i++) {
		if(cTIMER_CheckTimerExceed(packetQueue[i].retryTime) && (packetQueue[i].packetID != 0)) {  
			if(packetQueue[i].retryCount >= struct_DevConfig.transmitRetry) {
				...
				printDebug_Semphr(INFO, "<PKTQUEUE_Retransmit> FID[%04X] type : %02X Time : %d \r\n", packetQueue[i].packetID, frameType, timestamp);
				if(frameType == STATREPC) {
					if(packetQueue[i].packetID == _DataLog_WaitID) {
						_DataLog_WaitID = 0x00;
						printDebug_Semphr(INFO, "%80s<PKTQUEUE_Retransmit> _DataLog_WaitID = 0x00 \r\n", "");
					}else {
						if(flag_isSDInstall) {
				...
				if(!xQueueSend(Queue_PacketHTTPToGSM, &packet_data, 0)) {
...
/**
  ...
  * @retval Packet ID
  */
uint16_t ucPKTQUEUE_GenerateID(PACKET_QUEUE *packetQueue) {
                 
	uint16_t id;

	do {
 		id = (((rand() % 255) & 0xFF) << 8) + ((rand() % 255) & 0xFF);
	}while(ucPKTQUEUE_isIdExist(packetQueue, id) || (id == 0x0000) || (id == 0xFFFF));
