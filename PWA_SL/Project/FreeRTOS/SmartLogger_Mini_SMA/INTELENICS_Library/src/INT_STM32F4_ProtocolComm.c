...
uint16_t		_DataLog_WaitID = 0x00;
uint16_t		_EventLog_WaitID = 0x00;
uint16_t		_ConfigLog_WaitID = 0x00;
...
PTC_RES xPROCOMM_ProcessPacket(uint8_t *buf, uint16_t length, uint8_t source) {
	...
	uint16_t 	frameID;																																/* Frame id */
	uint8_t 	frameType;																															/* Frame type */
	uint16_t 	MTI;																																		/* Message type indicator */
	uint8_t		MTI_class;																															/* MTI-SMA Message class */
	uint8_t		MTI_function;																														/* MTI-SMA Message function */
	uint8_t		MTI_hostrequset;																												/* MTI-SMA Message Host Request */ 
	uint16_t 	DataelementsLen;     																										/* Data elements length */
	uint16_t  i;
...
//	}
	...
	MTI							= (buf[INDEX2_MTI] << 8) + buf[INDEX2_MTI+1];
	frameID     		= (buf[INDEX2_FRAMEID] << 8) + buf[INDEX2_FRAMEID+1]; 
	frameType   		= buf[INDEX2_FRAMETYPE];
	
	MTI_class				=	MTI >> 8;
	MTI_function 		= (MTI & 0xFF) >> 4;
	MTI_hostrequset	=	(MTI & 0x0F);
	
	/* 														MTI		FrameID		TerminalID  TransactionTime */
	DataelementsLen 		= (length  -2     -2 			-ADDR_LEN			-4);
...
// 	}
	...
	/* Time Sync */
	if(vPTCHANDLE_TimeSync(&buf[INDEX2_TIME])) {
		/* fpPROCOMM_HandleTimeSync Failed */
		printDebug_Semphr(MINOR_ERR, "fpPROCOMM_HandleTimeSync Failed. \r\n");
	}
	
	if(MTI_hostrequset == 1) {
		xPROCOMM_SendConfigManageREQ(source);
	}
	
	switch(frameType) {
	/*------------------------------- Command Packet -------------------------------*/
	case CONFMANC :                                                       						/* [0x4A] Configuration Management CMD */
		printDebug_Semphr(INFO, "-->>> CONFIGURATION_MANAGEMENT CMD : FID[%04X][%02X]\r\n", frameID, buf[INDEX2_CONFIG_CNFG]);
...
// 		}else if(buf[INDEX2_CONFIG_ACCD] == CONF_ACCD_WRITE) {			/* ACCD: WRITE */
			...
			if(res == 0) {
				/* Handle Success */
				ptc_res = xPROCOMM_SendConfigManageACK(frameID, CONF_ACCD_WRITE, buf[INDEX2_CONFIG_CNFG], STAT_SUCCESS,NULL, 0, source);
				if(ptc_res != PTC_OK) {
					printDebug_Semphr(MINOR_ERR, "X<<-- CONFIGURATION_MANAGEMENT ACK Failed Err(PTC_%d) \r\n", ptc_res);
				}
				...
				ptc_res = xPROCOMM_SendConfigManageACK(frameID, CONF_ACCD_WRITE, buf[INDEX2_CONFIG_CNFG], STAT_ERR, NULL, 0, source);
				if(ptc_res != PTC_OK) {
					printDebug_Semphr(MINOR_ERR, "X<<-- CONFIGURATION_MANAGEMENT ACK Failed Err(PTC_%d) \r\n", ptc_res);
				}
			}
...
// 		}
	...
	case LOGMANIC :																																		/* [0x6A] Log Manipulation CMD */
		printDebug_Semphr(INFO, "-->>> LOG_MANIPULATION CMD : FID[%02X]\r\n", buf[INDEX2_FRAMEID]);
	...
	case NETMAINC :                                                       						/* [0x7A] Network Maintenance CMD */
		printDebug_Semphr(INFO, "-->>> NETWORK_MAINTENANCE CMD : CMD[%02X]\r\n", buf[INDEX2_NETMT_CMD]);
...
//					}
	...
	case RMDCTRLC :                                                       						/* [0x8A] Remote Device Control CMD  */
		printDebug_Semphr(INFO, "-->>> REMOTE_DEVICE_CONTROL CMD : FID[%02X]\r\n", buf[INDEX2_FRAMEID]);
	...
	case HOSTRESA :																																/* [0x00] Host Reponse Ack */
		switch(buf[INDEX_RESPONSE_CMD]) {
			case EVENREPA :                                                        						/* [0x3B] Event Report ACK */
				printDebug_Semphr(INFO, "-->>> EVENT_REPORT ACK : FID[%04X]\r\n", frameID);
			...
			case CONFMANA :                                                        						/* [0x4B] Configuration Management ACK */ 
				printDebug_Semphr(INFO, "-->>> CONFIGURATION_MANAGEMENT ACK : FID[%04X]\r\n", frameID);
			...
			case STATREPC :                                                        						/* [0x9B] Data&Status Report ACK */  
				printDebug_Semphr(INFO, "-->>> DATA&STATUS_REPORT ACK : FID[%04X]\r\n", frameID);
				...
				break;
			default:  
				printDebug_Semphr(INFO, "\r\n-->>> RECEIVE INT_COMM> Tag response not define or not reference yet (0x%02X)!!\r\n\r\n", buf[INDEX_RESPONSE_CMD]);
//			INT_sendProtocolErrorFeedbackAck(frameID, INVALID_FTYPE);                
	...
	case PROTOERROR :                                                      						/* [0xEB] Protocol Error Feedback */    
		printDebug_Semphr(INFO, "\r\n-->>> PROTOCOL_ERROR_FEEDBACK_ACK : FRAME ID[%04X] ERROR ID[0x%02X]\r\n\r\n", frameID, buf[31] );                                 
		break;
...
PTC_RES xPROCOMM_SendConfigManageCMD(uint8_t accd, uint8_t recordType, uint32_t timestamp, uint8_t cnfg, void *spec, uint16_t specLen, uint32_t timeout) {
	...
	printDebug_Semphr(INFO, "<<<-- CONFIGURATION_MANAGEMENT CMD : FID[%04X]\r\n", frameID);
...
/**
  ...
  * @retval Return protocol function code
  */
PTC_RES xPROCOMM_SendLogManipulateACK(uint16_t frameID, uint8_t accd, uint8_t stat, uint8_t dest) {
	...
	printDebug_Semphr(INFO, "<<<-- LOG_MANIPULATION ACK [%s] : FID[%04X]\r\n", (dest==GSM_PORT)?"GSM":"DEBUG", frameID);
...
/**
  ...
  * @retval Return protocol function code
  */
PTC_RES xPROCOMM_SendNetworkMaintACK(uint16_t frameID, uint8_t cmd, uint8_t stat, uint8_t *spec, uint8_t specLen) {
	...
	printDebug_Semphr(INFO, "<<<-- NETWORK_MAINTENANCE ACK : CMD[%04X]\r\n", cmd);
...
/**
  ...
  * @retval Return protocol function code
  */
PTC_RES xPROCOMM_SendRemoteCtrlACK(uint16_t frameID, uint8_t cid, uint8_t stat, uint8_t dest) {
	...
	printDebug_Semphr(INFO, "<<<-- REMOTE_DEVICE_CONTROL ACK [%s] : FID[%04X]\r\n", (dest==GSM_PORT)?"GSM":"DEBUG", frameID);
...
/**
  ...
  * @retval Return protocol function code
  */
PTC_RES xPROCOMM_SendConfigManageACK(uint16_t frameID, uint8_t accd, uint8_t cnfg, uint8_t stat, uint8_t *spec, uint16_t specLen, uint8_t dest) {
	...
	printDebug_Semphr(INFO, "<<<-- CONFIGURATION_MANAGEMENT ACK : FID[%04X] (%s)\r\n", frameID, (dest==GSM_PORT)?"GSM":"DEBUG");
	
	MTI = HOREMESC<<8 ;																											// Host Request Message
	
	if(accd == CONF_ACCD_WRITE) {
		// 				 TYP  LEN CNFG STAT
		frameLen = +1   +1   +1   +1;
		
		frameData[index++] = 0x4B;                                              // Frame Type
		frameData[index++] = 0x02;																							// Lenght of value
		frameData[index++] = cnfg;                                              // CNFG
		frameData[index++] = stat;                                              // STAT
		
	}else if(accd == CONF_ACCD_READ) {
		// 				 TYP LEN CNFG  SPEC_LEN
		frameLen = +1   +1   +1  +specLen;
		
		frameData[index++] = CONFMANC;                                         	// Frame Type
		frameData[index++] = specLen;																						// Lenght of value
		frameData[index++] = cnfg;                                              // CNFG
		if(specLen > 0) {
			memcpy(&frameData[index], spec, specLen);                  						// Specific Structure
			index += specLen;  
	...
	if(dest == GSM_PORT) {
		ptc_res = xPROCOMM_SendPacketToGSM(timestamp, frameID, MTI, frameData, frameLen, 0);
		if(ptc_res != PTC_OK) {               
			return PTC_SEND_FAIL;
		}
	}else if(dest == DEBUG_PORT) {
...
/*==========================================================================================*/
/**
  * @brief  Send ConfigurationManagement Request
  * @param  
  * @retval Return protocol function code
  */
PTC_RES xPROCOMM_SendConfigManageREQ(uint8_t dest) {
	...
	printDebug_Semphr(INFO, "<<<-- CONFIGURATION_MANAGEMENT REQ : FID[%04X] (%s)\r\n", frameID, (dest==GSM_PORT)?"GSM":"DEBUG");
...
PTC_RES xPROCOMM_SendDataNStatusReportCMD(uint32_t timestamp, uint8_t recordType, uint8_t numStatSet, STATUS_SET *status, uint8_t numDatSet, DATA_SET *data, uint32_t timeout, uint8_t dest) {
	...
	frameID = ucPKTQUEUE_GenerateID(packet_Queue);
	printDebug_Semphr(INFO, "<<<-- DATA&STATUS_REPORT CMD [%s] : FID[%04X]\r\n", (dest==GSM_PORT)?"GSM":"DEBUG", frameID);

	/*  					TYP_S						LEN_S							STAT_SIZE											TYP_D					LEN_D						DAT_SIZE										*/
	frameLen =	+numStatSet		+numStatSet		+(numStatSet*sizeof(STATUS_SET))	+numDatSet		+numDatSet		+(numDatSet*sizeof(DATA_SET));
	...
	if(recordType == RECORD_RLT) {
		MTI = (TRANMESC<<8) + (REQUMESF<<4) ;
	}else {	/* Add data_log wait frame_id */
		MTI = (BATUMESC<<8) + (REQUMESF<<4) ;
		_DataLog_WaitID = frameID;
	}
																																			 
	for(i = 0; i < numStatSet; i++) {                                      								/* Status Set */	
		frameData[index++] = STATREPC;                                       								/* Status Report COMMAND */
		frameData[index++] = (sizeof(STATUS_SET));																					/* Length of status set */
		memcpy(&frameData[index], &status[i], sizeof(STATUS_SET));
		index += sizeof(STATUS_SET);
	}
	
	for(i = 0; i < numDatSet; i++) {                                       								/* Data Set */
		frameData[index++] = DATAREPC;                                        							/* Data Report COMMAND */
		frameData[index++] = (sizeof(DATA_SET));																						/* Length of data set */
		memcpy(&frameData[index], &data[i], sizeof(DATA_SET));
		index += sizeof(DATA_SET);
	}
	
	if(dest == GSM_PORT) {
		ptc_res = xPROCOMM_SendPacketToGSM(timestamp, frameID, MTI, frameData, frameLen, timeout);
		if(ptc_res != PTC_OK) {               
			return PTC_SEND_FAIL;
		}
	}else if(dest == DEBUG_PORT) {
...
PTC_RES xPROCOMM_SendDataNStatusLog(uint32_t timestamp, uint16_t fTotal, uint16_t fNo, uint8_t numStatSet, STATUS_SET *status, uint8_t numDatSet, DATA_SET *data, uint32_t timeout) {
	...
	printDebug_Semphr(INFO, "<<<-- DATA&STATUS_LOG \r\n");
...
PTC_RES xPROCOMM_SendEventReportCMD(uint32_t timestamp, uint8_t recordType, uint8_t eid, uint32_t edat, uint32_t timeout) {
	...
	frameID = ucPKTQUEUE_GenerateID(packet_Queue);
	printDebug_Semphr(INFO, "<<<-- EVENT_REPORT CMD : FID[%04X]\r\n", frameID);
...
/**
  ...
  * @retval None
  */
PTC_RES xPROCOMM_SendPacketToGSM(uint32_t timestamp, uint16_t frameID, uint16_t MTI, uint8_t *data, uint32_t dataLen, uint32_t timeout) {
	...
	/* 				Start Len 	MTI  SendAddr	  TIM	 FID		Payload 	CheckSum   */
	packetLen = 1		+2     +2	 +ADDR_LEN   +4	 +2			+dataLen    +1;
	...
	sendPacket[index++] = (frameLen >> 8) & 0xFF;                                      		/* Packet Length - MSB */
  sendPacket[index++] = frameLen & 0xFF;                                                /*               - LSB */
	sendPacket[index++] = (MTI >> 8) & 0xFF;                                      				/* Message Type indicator - MSB */
  sendPacket[index++] = MTI & 0xFF;                                                			/*               					- LSB */
	memcpy(&sendPacket[index], &struct_info.serialNumber[0], ADDR_LEN);                 	/* Sender Address */
	index += ADDR_LEN;
// 	memcpy(&sendPacket[index], &struct_info.destSerial[0], ADDR_LEN);                     /* Receiver Address */
// 	index += ADDR_LEN;
	memcpy(&sendPacket[index], &timestamp, sizeof(timestamp));														/* Timestamp */
	index += sizeof(timestamp);
	sendPacket[index++] = (frameID >> 8) & 0xFF;                                      		/* Frame ID - MSB */
  sendPacket[index++] = frameID & 0xFF;                                                	/*          - LSB */
	
	memcpy(&sendPacket[index], data, dataLen);                                            /* Payload */
	index += dataLen;
	sendPacket[index] = ucPROCOMM_Checksum(&sendPacket[3], frameLen);                			/* Checksum */
	
	if((timeout > 0) && (frameID != 0xFFFF)) {
...
PTC_RES xPROCOMM_SendHTTPPacket(uint32_t timestamp, uint8_t numStatSet, STATUS_SET *status, uint8_t numDatSet, DATA_SET *data) {
	...
	uint8_t headerHTTP[] = "http://164.115.22.29/SmartSend29.php?query=";
	uint8_t		datetime[14];
	uint8_t 	res;
