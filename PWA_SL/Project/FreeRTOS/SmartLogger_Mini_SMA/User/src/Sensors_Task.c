...
//	float 						val_420mA				= 0.0;
	...
	struct_ConfigPulseSensor.sensor_enable = SENSOR_ENABLE;
			...
			if(cTIMER_CheckTimerExceed(samplingPulseTimer)) {
				...
				if(Flow_CalculateMethod == 0) {
					...
					if(_PulseData_Counter == 0) {																				/* For first times */
						...
						pulse_buf[_index_pulseData] = TM_BKPSRAM_Read32(0x08);
					}else {																															/* Other times */
						/* m³/min to m³/hr */
...
// 					_Sensor_Data.dataPULSEFlow = sum_flow/(float)(_PulseData_Counter);
        ...
        TM_BKPSRAM_Write32(0x08, _Sensor_Data.dataPULSEFlow);
