...
void vManager_Task(void *pvParameters) {
	...
	struct_DevConfig.debug_level = MAJOR_ERR | MINOR_ERR | DESCRIBE | INFO;
	struct_ConfigWorking.minBattery = 8.0;
	struct_DevConfig.retransmitInterval = 100;
	struct_DevConfig.transmitRetry = 0;
	...
	while(1) {
		...
		if(_ConfigCmdACK == CONF_ACK_OK) {
			/* Write serial number at backup SRAM location 0x00-0x08 */
			for(i=0;i<sizeof(struct_info.serialNumber);i++){
				TM_BKPSRAM_Write8(i, struct_info.serialNumber[i]);
			}
