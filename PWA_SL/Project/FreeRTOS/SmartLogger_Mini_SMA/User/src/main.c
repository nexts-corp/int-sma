...
int main(void) {
	...
	unsigned char				check_HEELO[] 	= "HELLO_WORLD";
	unsigned char				check_text[8];
	unsigned char				read_text[8];
	float temp;
...
//   TM_DS1307_Init();
	...
	if(fatfs_res == FR_OK) {
		...
		/* Read serial backup from SRAM */
		printDebug(DESCRIBE, "[SRAM]: text = ");
		for(i=0;i<sizeof(struct_info.serialNumber);i++) {
			printDebug(DESCRIBE, "%02X ", TM_BKPSRAM_Read8(i));
			check_text[i] = TM_BKPSRAM_Read8(i);
		}
		printDebug(DESCRIBE, "\r\n");
		...
		if(fatfs_res == FR_OK) {
			...
			printDebug(DESCRIBE, "[SRAM]: read_text = ");
			for(i=0;i<sizeof(read_text);i++) {
				printDebug(DESCRIBE, "%02X ", read_text[i]);
			}
			printDebug(DESCRIBE, "\r\n");
	...
	/* EEPROM ----------------------------------------------------------------*/
	memcpy(read_text, "-----------", sizeof(check_text));			 	// clear read_text
	if(EEPROM_ReadBuf(CHKTEXT_BASEADDR, &read_text[0], sizeof(check_HEELO))) {
		isEEPROMError |= true;
		printDebug(DESCRIBE, "[EEPROM]: Read CheckText Error. \r\n");
	}else {
		if(memcmp(check_HEELO, read_text, sizeof(check_HEELO)) == 0) {
			...
			if(EEPROM_WriteBuf(CHKTEXT_BASEADDR, &check_HEELO[0], sizeof(check_HEELO))) {
...
static void SL_InitialDigitalOutput(void) {
	...
	/* VGSM Enable/Disable */
	GPIO_InitStruct.GPIO_Pin = GSM_VEN_PIN;
	GPIO_Init(GSM_VEN_PORT, &GPIO_InitStruct);
	/* GSM Module Reset */
	GPIO_InitStruct.GPIO_Pin = GSM_RESET_PIN;
	GPIO_Init(GSM_RESET_PORT, &GPIO_InitStruct);
	/* GSM Module PowerKey */
	GPIO_InitStruct.GPIO_Pin = GSM_PWR_PIN;
	GPIO_Init(GSM_PWR_PORT, &GPIO_InitStruct);
	/* GSM Module Status */
	GPIO_InitStruct.GPIO_Pin = GSM_STATUS_PIN;
	GPIO_Init(GSM_STATUS_PORT, &GPIO_InitStruct);
	/* GSM Module Net_Mode */
	GPIO_InitStruct.GPIO_Pin = GSM_NETMODE_PIN;
	GPIO_Init(GSM_NETMODE_PORT, &GPIO_InitStruct);
	/* GSM Module Sleep_IND */
	GPIO_InitStruct.GPIO_Pin = GSM_SLEEP_PIN;
	GPIO_Init(GSM_SLEEP_PORT, &GPIO_InitStruct);
	...
	// Select RF
	GPIO_SetBits(GSM_VEN_PORT, GSM_VEN_PIN);
