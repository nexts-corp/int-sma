...
void vGSM_PowerOff( void ) { 
	...
	GPIO_ResetBits( GSM_VEN_PORT, GSM_VEN_PIN ); 
}
...
#if GSM_USE_UC20
	vGSM_SendATcommand( ( uint8_t* )"AT+QIDEACT=1\r\n", 14 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 100 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+QIDEACT error\r\n" );
	
	vGSM_SendATcommand( ( uint8_t* )"AT+QICSGP?\r\n", 12 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 10 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+QICSGP? error\r\n" );
	
	vGSM_SendATcommand( ( uint8_t* )"AT+QICSGP=1,1,\"PWA.SCADA\",\"\",\"\",1\r\n", 35 );
	if ( ucGSM_waitForOk( (uint8_t *)"OK", 10 ) != GSM_ENOERR )
		printDebug_Semphr( INFO,"[GSM_TASK]: AT+QICSGP=1 error\r\n" );
...
void vGSM_CloseConnection( void ) {
	/* send at command close connection */
...
void vGSM_SendHTTPGET( uint8_t* ucURL, uint16_t LenURL ) {
	...
	printDebug_Semphr( ALWAYS, "[GSM_TASK]: %s(%d)\r\n", ucURL, LenURL );
	vGSM_SendATcommand( (volatile uint8_t *)ucStrCommand, 20 );
	if( ucGSM_waitForOk( (uint8_t *)"CONNECT", 200 )== GSM_ENOERR ) {
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: Connect Http OK\r\n" );
	}
	else {
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: Connect Http error\r\n" );
	}
 	
	vGSM_SendATcommand( ucURL, LenURL );
	if( ucGSM_waitForOk( (uint8_t *)"OK" ,200 )== GSM_ENOERR ) {
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: input URL OK\r\n" );
	}
	else {
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: input URL error\r\n" );
	}
 	
	vGSM_SendATcommand( (volatile uint8_t *)"AT+QHTTPGET=60\r\n", 16 );
	if( ucGSM_waitForOk( (uint8_t *)"OK", 200 )== GSM_ENOERR ) {
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: GET OK\r\n" );
	}
	else {
		vGSM_SendATcommand( ESC, 2 );
		printDebug_Semphr( ALWAYS, "[GSM_TASK]: GET error\r\n" );
	}
// 	
